...
import com.xiyuxian.api.aliyunai.model.CreateOutPaintingTaskRequest;
import com.xiyuxian.api.aliyunai.model.CreateOutPaintingTaskResponse;
import com.xiyuxian.controller.upload.PictureUploadTemplate;
import com.xiyuxian.domain.Picture;
import com.xiyuxian.domain.Space;
import com.xiyuxian.picture.*;
import com.xiyuxian.domain.UploadPictureResult;
...
import com.xiyuxian.manager.CosManager;
import com.xiyuxian.service.*;
import com.xiyuxian.mapper.PictureMapper;
import com.xiyuxian.utils.ColorSimilarUtils;
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (spaceId != null) {
            ...
            if (space.getTotalCount() >= space.getMaxCount()) {
                throw new BusinessException(ErrorCode.OPERATTION_ERROR, "空间条数不足");
            }
            if (space.getTotalSize() >= space.getMaxSize()) {
                throw new BusinessException(ErrorCode.OPERATTION_ERROR, "空间大小不足");
            }
        }
...
//        }
        ...
        // ThrowUtils.throwIf(!result, ErrorCode.OPERATTION_ERROR, "图片上传失败");
        ...
        transactionTemplate.execute(status -> {
            // 插入数据
            boolean result = this.saveOrUpdate(picture);
            ThrowUtils.throwIf(!result, ErrorCode.OPERATTION_ERROR, "图片上传失败，数据库操作失败");
            if (finalSpaceId != null) {
                ...
                boolean update = spaceService.lambdaUpdate()
                        ...
                        .update();
                ThrowUtils.throwIf(!update, ErrorCode.OPERATTION_ERROR, "额度更新失败");
            }
            return picture;
        });
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        try {
            ...
            log.error("获取页面失败", e);
            throw new BusinessException(ErrorCode.OPERATTION_ERROR, "获取页面失败");
        }
        Element div = document.getElementsByClass("dgControl").first();
        if (ObjUtil.isNull(div)) {
            throw new BusinessException(ErrorCode.OPERATTION_ERROR, "获取元素失败");
        }
    ...
    private void fillPictureWithNameRule(List<Picture> pictureList, String nameRule) {
        ...
        try {
            ...
            log.error("名称解析错误", e);
            throw new BusinessException(ErrorCode.OPERATTION_ERROR, "名称解析错误");
        }
    }
